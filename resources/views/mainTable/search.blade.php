					...
					<div class="widget category-list">
                        ...
                        <ul class="category-list">
                            @foreach ( $categoriesAll as $categoryAll)
                                <li><a href="categories/{{ $categoryAll->id }}">{{ $categoryAll->name}} <span>{{$categoryAll->companies->count()}}</span></a></li>
                            @endforeach
                        </ul>
                    </div>
				</div>
                                ...
                                <div class="col-sm-12 col-lg-4 col-md-6">
                                    ...
                                    <div class="product-item bg-light">
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                <h4 class="card-title"><a href="{{ url('/company/' . $company->id) }}">{{$company->name}}</a></h4>
                                                @foreach ($company->categories as $singleCategories)
                                                    <ul class="list-inline product-meta">
                                                        <li class="list-inline-item">
                                                            <a href="{{ url('/categories/' . $singleCategories->id) }}"><i class="fa fa-folder-open-o"></i>{{ $singleCategories->name }}</a>
                                                        </li>
                                                    </ul>
                                                @endforeach
